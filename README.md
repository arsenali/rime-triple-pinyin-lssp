...
方案所需全部文件已打包，可在[网盘](http://www.lssp.ysepan.com/)或QQ群([150478288](https://jq.qq.com/?_wv=1027&k=5wf1uTQ))下载补丁包直接导入。
...
另外，还可使用笔画码筛选去重，用法类似于搜狗拼音，笔画码相当于间接辅助码。
...
3. 部分方言中的音变现象，例如：地上一`yí`个猴，树上七`qí`个猴，一共八`bá`个猴，输入时仍按原声调输入，即：地上一`yī`个猴，树上七`qī`个猴，一共八`bā`个猴；再如：别说话`biè shuō huà`，输入时应打`bié shuō huà`。
