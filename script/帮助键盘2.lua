...
如此，在输入第3码之后便得到了唯一的一个带调拼音音节，不会有重复音节。
  ...
  {"便捷输入", [[
[无声调模式]
如果不想区分声调，可长按或上滑T键切换到无声调模式，此时在第3码可用TGB三键输入无声调音节。
打字过程中也可随时把已输入的带调音节转换为无声调音节，右滑“全码”键进行转换，而不必切换到无声调模式重新输入。

[首码简拼、前两码简拼、全码输入]
打字过程中，右滑“简码”键，检索首码简拼。
打字过程中，点击“简码”键，强制检索前两码简拼。
打字过程中，点击“全码”键，强制检索全码字。
...
[尖音输入]
方言中有尖音的朋友可打尖音字以降低重码率，z开头的尖音音节以“b+第3、4类韵母”（bu除外）输入，例如：尖zian=buan，绝züe=büe；同理，c开头的尖音音节以“p+第3、4类韵母”（pu除外）输入，s开头的尖音音节以“m+第3、4类韵母”（mu除外）输入。
